...
export const createCampaign = async (
  ...
  try {
    ...
    const bStart = typeof businessStartHour === "number" ? businessStartHour : null;
    const bEnd = typeof businessEndHour === "number" ? businessEndHour : null;
    ...
    const { campaign, emailJobs, campaignSenders, priorityJobs } = await prisma.$transaction(async (tx) => {
      ...
      for (let i = 0; i < recipients.length; i++) {
        ...
        await logContactActivity(contact.id, ActivityType.CAMPAIGN_ENROLLED, { 
          campaignId: campaign.id,
